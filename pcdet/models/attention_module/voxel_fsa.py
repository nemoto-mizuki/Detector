...
import math
from pcdet.models.attention_module.sa_block import SA_block
...
class VoxelContext3D_fsa(nn.Module):
    def __init__(self, model_cfg, grid_size, voxel_size, point_cloud_range, dropout=0.1):
        ...
        self.self_attn1 = SA_block(inplanes=self.model_cfg.IN_DIM, planes=self.model_cfg.NUM_BEV_FEATURES)
        self.self_attn2 = SA_block(inplanes=self.model_cfg.IN_DIM, planes=self.model_cfg.NUM_BEV_FEATURES)
    ...
    def forward(self, batch_dict):
        ...
        """
        encoded_spconv_tensor = torch.cat(batch_dict['encoded_spconv_tensor'], dim=0)
