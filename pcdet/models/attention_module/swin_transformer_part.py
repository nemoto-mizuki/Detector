...
class FeedForward(nn.Module):
    def __init__(self, cfg):
        ...
        self.ff = nn.Sequential(
            nn.Conv2d(in_channels, hidden, kernel_size=kernel_size, stride=stride, padding=padding),
            nn.GELU(),
            nn.Dropout2d(p=dropout),
            nn.Conv2d(hidden, out_channels, kernel_size=kernel_size, stride=stride, padding=padding),
            nn.Dropout2d(p=dropout)
        )
