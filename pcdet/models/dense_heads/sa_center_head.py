...
class SACenterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.key = nn.Sequential(
            nn.Conv2d(self.model_cfg.SHARED_CONV_CHANNEL, self.model_cfg.SHARED_CONV_CHANNEL, 1,stride=1),
            nn.BatchNorm2d(self.model_cfg.SHARED_CONV_CHANNEL),
            nn.ReLU(),
        )
        ...
        self.scale = self.model_cfg.SHARED_CONV_CHANNEL ** -0.5
    ...
    def forward(self, data_dict):
        ...
        k = k.unsqueeze(dim=1)
        q = self.quely(x)
        s = (k * q.transpose(-1, -2)).sum(2, keepdim=True)
        s = s * self.scale
        mask = self.softmax(s)
        v = self.value(x)
        output = torch.matmul(mask, v) + x
        x = x[:,0,...]
