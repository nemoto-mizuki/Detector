...
class WaymoDataset(DatasetTemplate):
    ...
    def get_sequence_data(self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False):
        ...
        pickup_index = np.arange(sequence_cfg.SAMPLE_OFFSET[0], sequence_cfg.SAMPLE_OFFSET[1])
        sample_idx_pre_list = np.clip(sample_idx + pickup_index*sequence_cfg.SAMPLE_INTERVAL, 0, 0x7FFFFFFF)
        ...
        for idx, sample_idx_pre in enumerate(sample_idx_pre_list):
            ...
            if sequence_cfg.get('ONEHOT_TIMESTAMP', False):
                ...
                points_pre = np.hstack([points_pre, ((sample_idx - sample_idx_pre)/sequence_cfg.SAMPLE_INTERVAL) * np.ones((points_pre.shape[0], 1)).astype(points_pre.dtype)])
    ...
    def __getitem__(self, index):
        ...
        input_dict = {
            ...
            'split_data': self.dataset_cfg.SEQUENCE_CONFIG.SPLIT_DATA if self.dataset_cfg.get('SEQUENCE_CONFIG', False) else False,
            'split_length': abs(max(self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET, key=abs))+1 if self.dataset_cfg.get('SEQUENCE_CONFIG', False) else 0,
            'split_interval': self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_INTERVAL if self.dataset_cfg.get('SEQUENCE_CONFIG', False) else False,
        }
